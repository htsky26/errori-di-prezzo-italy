...
CATEGORIE = [
    ("Electronics", "offerta sconto"),
    ("Computers", "offerta sconto"),
    ("VideoGames", "offerta sconto"),
    ("HomeAndKitchen", "offerta sconto"),
    ("Apparel", "offerta sconto"),
    ("Fashion", "offerta sconto"),
    ("HealthPersonalCare", "offerta sconto"),
    ("GardenAndOutdoor", "offerta sconto"),
    ("SportingGoods", "offerta sconto"),
]
...
def cerca_offerte(categoria, keywords, token):
    ...
    try:
        ...
        if items:
            logger.info(f"Esempio item: {str(items[0])[:400]}")
        return items
    except Exception as e:
...
def estrai_prodotto(item):
    try:
        ...
        try:
            listing = (item.get("OffersV2") or item.get("Offers") or {}).get("Listings", [{}])[0]
        except Exception:
            return None

        try:
            prezzo_attuale = parse_prezzo(listing.get("Price", {}).get("Amount"))
        except Exception:
            prezzo_attuale = None

        try:
            prezzo_originale = parse_prezzo(listing.get("SavingBasis", {}).get("Amount"))
        except Exception:
