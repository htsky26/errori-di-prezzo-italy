...
def cerca_offerte(categoria, keywords, token):
    ...
    try:
        ...
        if r.status_code != 200:
            ...
            return []
        data = r.json()
        # Creators API usa lowerCamelCase anche nella risposta
        items = (data.get("searchResult") or data.get("SearchResult") or {}).get("items") or \
                (data.get("searchResult") or data.get("SearchResult") or {}).get("Items") or []
        logger.info(f"Items trovati: {len(items)}")
        return items
    except Exception as e:
...
def estrai_prodotto(item):
    try:
        ...
        # Prezzo da offersV2.listings[0].price.money.amount
        prezzo_attuale   = None
        ...
        try:
            ...
            if listings:
                ...
                # saving basis (prezzo originale barrato)
