...
CATEGORIE = [
    ...
    ("Electronics",        "router 5g"),
    ("Electronics",        "range extender wifi"),
    ("Electronics",        "ripetitore wifi"),
    ("Electronics",        "tp-link"),
    ("Electronics",        "netgear"),
    ("Electronics",        "asus router"),
    ("Electronics",        "fritz box"),
    ("Electronics",        "access point wifi"),
    ("Computers",          "switch di rete"),
    # Stampa 3D e filamenti
    ("Electronics",        "filamento PLA"),
    ("Electronics",        "filamento stampa 3D"),
    ("Electronics",        "PLA filament 1.75"),
    ("Electronics",        "stampante 3D"),
    ("Electronics",        "filamento PETG"),
    ("Electronics",        "filamento ABS"),
    ("Computers",          "resina stampante 3D"),
    ("Computers",          "filamento PLA 1kg"),
]

# ── Database con scadenza ─────────────────────────────────────────────────────
def carica_pubblicati():
    """Carica ASIN pubblicati, scartando quelli più vecchi di GIORNI_SCADENZA"""
    ...
    with open(DB_PATH, "r") as f:
        for line in f:
            ...
            if len(parti) == 2:
                ...
                # Vecchio formato senza data — scarta
                pass
    # Riscrivi il file solo con le righe valide (pulizia automatica)
    with open(DB_PATH, "w") as f:
        f.write("\n".join(righe_valide) + "\n" if righe_valide else "")
    logger.info(f"Database: {len(pubblicati)} ASIN attivi (scaduti rimossi)")
    return pubblicati
...
def cerca_offerte(categoria, keywords, token):
    ...
    try:
        ...
        logger.info(f"Search '{keywords}': status {r.status_code}")
        if r.status_code != 200:
            logger.error(f"Search error: {r.text[:200]}")
            return []
        data = r.json()
        items = (data.get("searchResult") or data.get("SearchResult") or {})
        risultati = items.get("items") or items.get("Items") or []
        logger.info(f"  Items trovati: {len(risultati)}")
        return risultati
    except Exception as e:
...
def main():
    ...
    for categoria, keywords in CATEGORIE:
        logger.info(f"Scansione: {keywords}")
