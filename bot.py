...
# ── Configurazione ────────────────────────────────────────────────────────────
AMAZON_ACCESS_KEY  = os.environ["AMAZON_ACCESS_KEY"]   # credential_id
AMAZON_SECRET_KEY  = os.environ["AMAZON_SECRET_KEY"]   # credential_secret
AMAZON_PARTNER_TAG = os.environ["AMAZON_PARTNER_TAG"]  # erroridipr083-21
...
def get_access_token():
    """Ottieni Bearer token OAuth2 da Creators API"""
...
def cerca_offerte(categoria, token):
    """Cerca prodotti scontati tramite Creators API"""
    url = "https://affiliate-program.amazon.it/api/2/products/search"
    headers = {
    ...
    try:
        ...
        if r.status_code != 200:
            logger.error(f"Search error: {r.text[:300]}")
            return []
        data = r.json()
...
def main():
    init_db()
    logger.info("🚀 Avvio scansione Creators API...")

    token = get_access_token()
    if not token:
        logger.error("❌ Impossibile ottenere access token — credenziali errate o non autorizzate")
        return
